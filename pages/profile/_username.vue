...
export default {
  ...
  methods: {
    async toggleFollow(profile) {
      profile.followLock = true;
      try {
        const {
          data: {
            profile: { following },
          },
        } = await (profile.following ? unfollow : follow)(profile.username);
        profile.following = following;
      } catch (error) {
        console.warn("followFailed", error);
      }
      profile.followLock = false;
    },
  },
